...
#Varying Parameters
lx1List="5"			#Element Size?
elmsPerCoreList="4"		#Elements per core?
partsPerCoreList="10"		#Particles per Processor
...
do
  ...
  do
	...
	cd $root
	mkdir $machineName'_'$numCore'cores_'$lx1'lx1_'$elmsPerCore'elmsPerCore'
	#vulcan_tst18_generate.sh
	cp input_SIZE uniform.* $machineName'_tst18_generate.sh' clean.sh $machineName'_'$numCore'cores_'$lx1'lx1_'$elmsPerCore'elmsPerCore'
	cd $machineName'_'$numCore'cores_'$lx1'lx1_'$elmsPerCore'elmsPerCore'
	cp input_SIZE SIZE
	...
	/g/g19/trokon/workspace/tst_2018_nek5000_lpm_benchmarking_repo/vulcan/bin/genbox < gbox.in
	mv box.re2 uniform.re2
	echo "uniform" > gmap.in
	echo "0.2" >> gmap.in
	/g/g19/trokon/workspace/tst_2018_nek5000_lpm_benchmarking_repo/vulcan/bin/genmap < gmap.in
